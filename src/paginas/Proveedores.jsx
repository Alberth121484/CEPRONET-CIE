...
import ModalFormularioProveedor from '../components/ModalFormularioProveedor';
import ModalEliminarProveedor from '../components/ModalEliminarProveedor';
import { useState, useEffect } from 'react';

const Proveedores = () => {
  const { proveedores, alerta, handleModalProveedor, handleModalEditarProveedor, handleModalEliminarProveedor, mostrarAlerta } = useProveedores();
  ...
  const exportToPDF = () => {
    ...
    const tableData = proveedores/*.slice(offset, offset + pageSize)*/.map(proveedor => [
      proveedor.nombre,
      proveedor.direccion,
      proveedor.telefono,
      proveedor.email,
      ...
      <div>
        ...
        <ModalEliminarProveedor />
          {proveedores.length ? (
            <>
              <table className="w-full" id="proveedoresTable">
                ...
                <tbody className='h-200 overflow-auto'>
                  {proveedores/*.slice(offset, offset + pageSize)*/.map(proveedor => (
                     <tr className="hover:bg-gray-100 transition-colors border-b border-gray-300" key={proveedor._id}>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{proveedor.nombre}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{proveedor.direccion}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{proveedor.telefono}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>
                         <a href={`mailto:${proveedor.email}`} className='text-blue-500 italic'>
                         {proveedor.email}
                         </a>
                     </td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>
                         {proveedor.web ? (
                         <a href={`http://${proveedor.web}`} target="_blank" rel="noopener noreferrer" className='text-blue-500 underline'>
                             {proveedor.web}
                         </a>
                         ) : (
                         '-'
                     ...
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{proveedor.actividad}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{proveedor.descripcion}</td>
                     <td className='px-2 py-2 font-bold text-center'>
                         <div className='flex justify-center space-x-4'>
                             <div>
                                 <button
                                     onClick={ () => handleModalEditarProveedor(proveedor)}
                                     type='button'
                                     className="p-3 font-bold block mt-5 text-center"
                                 >
                             ...
                             <div>
                                     <button
                                         className='p-3 font-bold block mt-5 text-center'
                                         onClick={() => handleModalEliminarProveedor(proveedor)}
                                     >
            ...
            <p className="text-center text-gray-600 uppercase p-5">No hay proveedores aún</p>
          )}
        </div>
      </div>
    </>
