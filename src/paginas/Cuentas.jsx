...
import ModalFormularioCuenta from '../components/ModalFormularioCuenta';
import ModalEliminarCuenta from '../components/ModalEliminarCuenta';
import { useState, useEffect } from 'react';
import { object } from 'prop-types';

const Cuentas = () => {
  const { cuentas, alerta, handleModalCuenta, handleModalEditarCuenta, handleModalEliminarCuenta, mostrarAlerta } = useCuentas();
  ...
  const exportToPDF = () => {
    ...
    doc.text('Cuentas', 40, 40);

    const tableHeaders = [['Número Cuenta', 'Nombre', 'Tipo', 'Saldo Inicial', 'Descripción']];
    const tableData = cuentas/*.slice(offset, offset + pageSize)*/.map(cuenta => [
      cuenta.numeroCuenta,
      cuenta.ultimosDigitosCuenta,
      cuenta.nobre,
      cuenta.tipo,
      cuenta.saldoInicial,
      cuenta.descripcion,
    ...
    <>
      ...
      <div>
        <div className="flex justify-between items-center">
          <h1 className="text-4xl font-black">Cuentas</h1>
          <button
        ...
        <ModalEliminarCuenta />
          {cuentas.length ? (
            <>
              <table className="w-full" id="cuentasTable">
                <thead className="bg-gray-800 text-white">
                  <tr>
                    <th className="py-2">Número Cuenta</th>
                ...
                <tbody className='h-200 overflow-auto'>
                  {cuentas/*.slice(offset, offset + pageSize)*/.map(cuenta => (
                     <tr className="hover:bg-gray-100 transition-colors border-b border-gray-300" key={cuenta._id}>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{'XXXX-XXXX-XXX-'+cuenta.ultimosDigitosCuenta}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{cuenta.banco ? bancos.find(banco => banco._id === cuenta.banco)?.nombre : ''}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{cuenta.nombre}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{cuenta.tipo ? tipos.find(tipo => tipo._id === cuenta.tipo)?.nombre : ''}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{cuenta.saldoInicial}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{cuenta.descripcion}</td>
                     <td className='px-2 py-2 font-bold text-center'>
                         <div className='flex justify-center space-x-4'>
                             <div>
                                 <button
                                     onClick={ () => handleModalEditarCuenta(cuenta)}
                                     type='button'
                                     className="p-3 font-bold block mt-5 text-center"
                                 >
                             ...
                             <div>
                                     <button
                                         className='p-3 font-bold block mt-5 text-center'
                                         onClick={() => handleModalEliminarCuenta(cuenta)}
                                     >
