...
import ModalFormularioBanco from '../components/ModalFormularioBanco';
import ModalEliminarBanco from '../components/ModalEliminarBanco';
import { useState, useEffect } from 'react';
import { object } from 'prop-types';

const Bancos = () => {
  const { bancos, alerta, handleModalBanco, handleModalEditarBanco, handleModalEliminarBanco, mostrarAlerta } = useBancos();
  ...
  const exportToPDF = () => {
    ...
    doc.text('Bancos', 40, 40);

    const tableHeaders = [['Nombre', 'Tipo', 'Numero Cuenta', 'Monto', 'Descripción']];
    const tableData = bancos/*.slice(offset, offset + pageSize)*/.map(banco => [
      banco.nombre,
      banco.tipo,
      banco.ultimosDigitosCuenta,
      banco.monto,
      banco.descripcion,
    ]);
    ...
    <>
      ...
      <div>
        <div className="flex justify-between items-center">
          <h1 className="text-4xl font-black">Bancos</h1>
          <button
        ...
        <ModalEliminarBanco />
          {bancos.length ? (
            <>
              <table className="w-full" id="bancosTable">
                ...
                <tbody className='h-200 overflow-auto'>
                  {bancos/*.slice(offset, offset + pageSize)*/.map(banco => (
                     <tr className="hover:bg-gray-100 transition-colors border-b border-gray-300" key={banco._id}>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{banco.nombre}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{banco.tipo && tipos.find(tipo => tipo._id === banco.tipo).nombre}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{'XXX XXX '+banco.ultimosDigitosCuenta}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{banco.monto}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{banco.descripcion}</td>
                     <td className='px-2 py-2 font-bold text-center'>
                         <div className='flex justify-center space-x-4'>
                             <div>
                                 <button
                                     onClick={ () => handleModalEditarBanco(banco)}
                                     type='button'
                                     className="p-3 font-bold block mt-5 text-center"
                                 >
                             ...
                             <div>
                                     <button
                                         className='p-3 font-bold block mt-5 text-center'
                                         onClick={() => handleModalEliminarBanco(banco)}
                                     >
            ...
            <p className="text-center text-gray-600 uppercase p-5">No hay Bancos aún</p>
          )}
        </div>
      </div>
    </>
