...
import ModalFormularioNegocio from '../components/ModalFormularioNegocio';
import ModalEliminarNegocio from '../components/ModalEliminarNegocio';
import { useState, useEffect } from 'react';

const Negocios = () => {
  const { negocios, alerta, handleModalNegocio, handleModalEditarNegocio, handleModalEliminarNegocio, mostrarAlerta } = useNegocios();
  ...
  const exportToPDF = () => {
    ...
    doc.text('Negocios', 40, 40);

    const tableHeaders = [['Nombre', 'Dirección', 'Teléfono', 'Email', 'Web', 'Actividad', 'Descripción']];
    const tableData = negocios/*.slice(offset, offset + pageSize)*/.map(negocio => [
      negocio.nombre,
      negocio.direccion,
      negocio.telefono,
      negocio.email,
    ...
    <>
      ...
      <div>
        <div className="flex justify-between items-center">
          <h1 className="text-4xl font-black">Negocios</h1>
          <button
        ...
        <ModalEliminarNegocio />
          {negocios.length ? (
            <>
              <table className="w-full" id="negociosTable">
                ...
                <tbody className='h-200 overflow-auto'>
                  {negocios/*.slice(offset, offset + pageSize)*/.map(negocio => (
                     <tr className="hover:bg-gray-100 transition-colors border-b border-gray-300" key={negocio._id}>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{negocio.nombre}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{negocio.direccion}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{negocio.telefono}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>
                         <a href={`mailto:${negocio.email}`} className='text-blue-500 italic'>
                         {negocio.email}
                         </a>
                     </td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>
                         {negocio.web ? (
                         <a href={`http://${negocio.web}`} target="_blank" rel="noopener noreferrer" className='text-blue-500 underline'>
                             {negocio.web}
                         </a>
                         ) : (
                         '-'
                     ...
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{negocio.actividad}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{negocio.descripcion}</td>
                     <td className='px-2 py-2 font-bold text-center'>
                         <div className='flex justify-center space-x-4'>
                             <div>
                                 <button
                                     onClick={ () => handleModalEditarNegocio(negocio)}
                                     type='button'
                                     className="p-3 font-bold block mt-5 text-center"
                                 >
                             ...
                             <div>
                                     <button
                                         className='p-3 font-bold block mt-5 text-center'
                                         onClick={() => handleModalEliminarNegocio(negocio)}
                                     >
            ...
            <p className="text-center text-gray-600 uppercase p-5">No hay negocios aún</p>
          )}
        </div>
      </div>
    </>
