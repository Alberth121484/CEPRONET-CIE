...
import ModalFormularioTipo from '../components/ModalFomularioTipo';
import ModalEliminarTipo from '../components/ModalEliminarTipo';
import { useState, useEffect } from 'react';

const Tipos = () => {
  const { tipos, alerta, handleModalTipo, handleModalEditarTipo, handleModalEliminarTipo, mostrarAlerta } = useTipos();
  ...
  const exportToPDF = () => {
    ...
    doc.text('Tipos', 40, 40);

    const tableHeaders = [['Nombre', 'Tipo', 'Numero Cuenta', 'Monto', 'Descripción']];
    const tableData = tipos/*.slice(offset, offset + pageSize)*/.map(tipo => [
      tipo.nombre,
      tipo.tipo,
      tipo.ultimosDigitosCuenta,
      tipo.monto,
      tipo.descripcion,
    ]);
    ...
    <>
      ...
      <div>
        <div className="flex justify-between items-center">
          <h1 className="text-4xl font-black">Tipos</h1>
          <button
        ...
        <ModalEliminarTipo />
          {tipos.length ? (
            <>
              <table className="w-full" id="tiposTable">
                ...
                <tbody className='h-200 overflow-auto'>
                  {tipos/*.slice(offset, offset + pageSize)*/.map(tipo => (
                     <tr className="hover:bg-gray-100 transition-colors border-b border-gray-300" key={tipo._id}>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{tipo.codigo}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{tipo.nombre}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{tipo.naturaleza}</td>
                     <td className='px-2 py-2 font-bold text-center border-r border-gray-300'>{tipo.descripcion}</td>
                     <td className='px-2 py-2 font-bold text-center'>
                         <div className='flex justify-center space-x-4'>
                             <div>
                                 <button
                                     onClick={ () => handleModalEditarTipo(tipo)}
                                     type='button'
                                     className="p-3 font-bold block mt-5 text-center"
                                 >
                             ...
                             <div>
                                     <button
                                         className='p-3 font-bold block mt-5 text-center'
                                         onClick={() => handleModalEliminarTipo(tipo)}
                                     >
            ...
            <p className="text-center text-gray-600 uppercase p-5">No hay Tipos aún</p>
          )}
        </div>
      </div>
    </>
